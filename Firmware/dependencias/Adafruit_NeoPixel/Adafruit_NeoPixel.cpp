...
// ESP8266 ----------------------------------------------------------------
  ...
  uint8_t hackBrigthness[numBytes];

  uint16_t scale;
  if( (brightness >= 250) || (brightness == 0) )
      scale = 250;
  else
      scale = (((uint16_t)brightness << 8) - 1) / 254;

  for(uint16_t i=0 ; i<numBytes ; i+=3)
  {
      hackBrigthness[i]   = (pixels[i+1] * scale) >> 8 ;
      hackBrigthness[i+1] = (pixels[i]   * scale) >> 8 ;
      hackBrigthness[i+2] = (pixels[i+2] * scale) >> 8 ;
  }

  // ESP8266 show() is external to enforce ICACHE_RAM_ATTR execution
  espShow(pin, hackBrigthness, numBytes, is800KHz);
...
void Adafruit_NeoPixel::setBrightness(uint8_t b) {
  ...
  if(true) { // Compare against prior value
...
uint8_t* Adafruit_NeoPixel::getDataArray()
{
    return pixels;
}
